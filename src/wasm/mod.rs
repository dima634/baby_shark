...
use crate::{
    algo::merge_points::merge_points, helpers::aliases::Vec3, mesh::{corner_table::{prelude::CornerTableD, VertexId}, traits::{Mesh as MeshTrait, VertexProperties}}
};
...
impl Mesh {
    pub fn from_vertices(position: &[f64]) -> Mesh {
        ...
        Self(CornerTableD::from_vertex_and_face_slices(&indexed.points, &indexed.indices))
    }
    ...
    fn vertex_indices_to_ids(&self, vertex_indices: impl ExactSizeIterator<Item = usize>) -> Vec<VertexId> {
    ...
    fn vertex_ids_to_indices(&self, vertex_ids: impl ExactSizeIterator<Item = VertexId>) -> Vec<usize> {
        let vertex_to_idx = self.vertex_to_index();
