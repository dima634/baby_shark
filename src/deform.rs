...
use faer::{
    linalg::{solvers::{ShapeCore, Solve}, svd::svd},
    sparse::{self as sp},
};
use nalgebra as na;
...
pub fn prepare_deform(
    ...
    let (coeffs, edge_weights) = compute_coeffs(mesh, &handle, &region_of_interest, &vertex_to_col);
    let size = region_of_interest.len() + handle.len();

    let Ok(a_mat) = sp::SparseColMat::try_new_from_triplets(size, size, &coeffs) else {
        return Err(PrepareDeformError::InternalError("failed to create sparse matrix"));
    };

    let Ok(factorization) = a_mat.sp_lu() else {
        return Err(PrepareDeformError::InternalError("failed to factorize matrix"));
    };
...
impl PreparedDeform {
    ...
    pub fn deform(&self, mesh: &CornerTableD, target: &HashMap<VertexId, Vec3d>) -> Result<CornerTableD, DeformError> {
        ...
        let handle_transformed: Vec<_> = self.handle
            .iter()
            .map(|vert| *target.get(vert).unwrap_or_else(|| mesh.vertex_position(vert)))
            .collect();
            ...
            for (i, &vi) in self.vertices().enumerate() {
                ...
                if i >= self.region_of_interest.len() {
                    ...
                    mesh.edges_around_vertex(&vi, |&edge| {
                        ...
                        if let Some(vj_idx) = self.vertex_to_col.get(&vj) {
...
fn compute_coeffs(mesh: &CornerTableD, handle: &[VertexId], roi: &[VertexId], vertex_to_col: &HashMap<VertexId, usize>) -> (Vec<Triplet>, EdgeAttribute<f64>) {
    ...
    for (i, vert) in roi.into_iter().enumerate() {
        ...
        mesh.edges_around_vertex(vert, |edge| {
            ...
            if !vertex_to_col.contains_key(&v1) || !vertex_to_col.contains_key(&v2) {
                return;
            }
