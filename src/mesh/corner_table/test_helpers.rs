...
pub fn create_flip_edge_sample_mesh() -> CornerTableF {
    ...
    let indices = vec![0, 1, 2, 2, 3, 0, 1, 6, 2, 2, 7, 3, 3, 4, 0, 0, 5, 1];

    CornerTableF::from_vertex_and_face_slices(&vertices, &indices)
}

pub fn assert_mesh_eq(
    mesh: &CornerTableF,
    expected_corners: &Vec<Corner>,
    expected_vertices: &Vec<Vertex<f32>>,
) {
    ...
    for i in 0..expected_corners.len() {
        assert_eq!(
            expected_corners[i], mesh.corners[i],
            "Testing corner: {}",
            i
        );
    }
