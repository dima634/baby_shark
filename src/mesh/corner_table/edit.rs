...
impl<TScalar: RealNumber> CornerTable<TScalar> {
    ...
    fn create_isolated_face_from_vertices(&mut self, v1: VertexId, v2: VertexId, v3: VertexId) -> (CornerId, CornerId, CornerId) {
...
impl<TScalar: RealNumber> CornerTable<TScalar> {
    ...
    fn split_inner_edge(&mut self, corner_id: CornerId, at: &Vec3<TScalar>) {        
        ...
        // Create new faces
        let (c6_idx, c7_idx, c8_idx) = self.create_isolated_face_from_vertices(v1_idx, new_vertex_id, v2_idx);
        let (c9_idx, c10_idx, c11_idx) = self.create_isolated_face_from_vertices(new_vertex_id, v3_idx, v2_idx);
    ...
    fn split_boundary_edge(&mut self, corner_id: CornerId, at: &Vec3<TScalar>) {
        // Existing corners and vertices that needs to be updated
        let mut walker = CornerWalker::from_corner(self, corner_id);
        let c0_idx = walker.previous_corner_id();
        let v1_idx = walker.corner().vertex();
        ...
        // Create new face
        let (c3_idx, c4_idx, c5_idx) = self.create_isolated_face_from_vertices(v1_idx, new_vertex_id, v2_idx);
...
impl<TScalar: RealNumber> SplitFaceAtPoint for CornerTable<TScalar> {
    fn split_face(&mut self, face: &Self::FaceDescriptor, point: Vec3<Self::ScalarType>) {
        ...
        let (c3_idx, c4_idx, c5_idx) = self.create_isolated_face_from_vertices(v1_idx, v2_idx, new_vertex_id);
        let (c6_idx, c7_idx, c8_idx) = self.create_isolated_face_from_vertices(v2_idx, v0_idx, new_vertex_id);
